        ...
        for entry in std::fs::read_dir(dir).map_err(|e| Error::ConfigError(e.to_string()))? {
                ...
                .unwrap_or(false)
            {
                ...
                let content = std::fs::read_to_string(&path).map_err(|e| {
                    Error::ConfigError(format!("Failed to read template {}: {}", path.display(), e))
                })?;
...
mod tests {
    ...
    fn test_simple_interpolation() {
        assert_eq!(convert_simple_interpolation("cat {path}"), "cat {{ path }}");
        assert_eq!(
    ...
    fn test_conditionals() {
        ...
        let result = pm
            .render_inline("Hello{% if show_extra %}, {{ name }}{% endif %}!", &ctx)
            .unwrap();
    ...
    fn test_struct_context() {
        ...
        let result = pm
            .render_inline("Position: ({{ x }}, {{ y }})", &ctx)
            .unwrap();
